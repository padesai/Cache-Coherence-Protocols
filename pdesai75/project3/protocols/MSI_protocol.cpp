...
{
	const char *block_states[7] = { "X","I","S","M","IS","MS","IM" };
...
void MSI_protocol::process_snoop_request(Mreq *request)
{
	...
	case MSI_CACHE_MS: do_snoop_MS(request); break;
...
inline void MSI_protocol::do_snoop_MS(Mreq *request)
{
	switch (request->msg) {
	case GETS:
	case GETM:
